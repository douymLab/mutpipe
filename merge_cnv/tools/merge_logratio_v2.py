...
for line in seg_f:
    ...
    chrom, start, end, fun = fields[0], fields[1], fields[2], fields[3]
...
for key in seg_dict0.keys():
    ...
    loss_num = 0
    bic_fun = ""
    if bic_median >= 0.2 and not math.isinf(bic_median):
        bic_fun = "gain"
        gain_num = gain_num + 1
    elif bic_median <= -0.2 and not math.isinf(bic_median):
        bic_fun = "loss"
        loss_num = loss_num + 1
    else:
        bic_fun = "-"
    ...
    dc_fun = ""
    if dc_median >= 0.2 and not math.isinf(dc_median):
        dc_fun = "gain"
        gain_num = gain_num + 1
    elif dc_median <= -0.2 and not math.isinf(dc_median):
        dc_fun = "loss"
        loss_num = loss_num + 1
    else:
        dc_fun = "-"
    ...
    freec_fun = ""
    if freec_median >= 0.2 and not math.isinf(freec_median):
        freec_fun = "gain"
        gain_num = gain_num + 1
    elif freec_median <= -0.2 and not math.isinf(freec_median):
        freec_fun = "loss"
        loss_num = loss_num + 1
    else:
        freec_fun = "-"
    ...
    line = "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(chrom, start, end, bic, dc, freec,bic_median,dc_median,freec_median,mean,fun,star,bic_fun, dc_fun, freec_fun, gain_num, loss_num, seg_dict0[key])
    print(line)
    out_f.write(line)
